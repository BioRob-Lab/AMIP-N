...
class DWT_1D(Module):
    ...
    def get_matrix(self):
        ...
        if torch.cuda.is_available():
            self.matrix_low = torch.Tensor(matrix_h).cuda().double()
            self.matrix_high = torch.Tensor(matrix_g).cuda().double()
        else:
            self.matrix_low = torch.Tensor(matrix_h).double()
            self.matrix_high = torch.Tensor(matrix_g).double()
