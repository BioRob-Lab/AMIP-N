...
class MultiWaveletCross(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, seq_len_q, seq_len_kv, modes, c=64,
        ...
        self.Lk = nn.Linear(ich, c * k).double()
        self.Lq = nn.Linear(ich, c * k).double()
        self.Lv = nn.Linear(ich, c * k).double()
        self.out = nn.Linear(c * k, ich).double()
        self.modes1 = modes
    ...
    def forward(self, q, k, v):
        ...
        for i in range(ns - self.L):
            ...
            dq, sq = Ud_q[i], Us_q[i]
            dv, sv = Ud_v[i], Us_v[i]
            Ud += [self.attn1(dq[0], dk[0], dv[0])[0] + self.attn2(dq[1], dk[1], dv[1])[0]]
            Us += [self.attn3(sq, sk, sv)[0]]
        v = self.attn4(q, k, v)[0]

        # reconstruct
        for i in range(ns - 1 - self.L, -1, -1):
            v = v + Us[i]
            v = torch.cat((v, Ud[i]), -1).double()
            v = self.evenOdd(v)
        v = self.out(v[:, :N, :, :].contiguous().view(B, N, -1).double())
        return v.contiguous()
    ...
    def wavelet_transform(self, x):
        ...
        d = torch.matmul(xa, self.ec_d.double())
        s = torch.matmul(xa, self.ec_s.double())
        return d, s

    def evenOdd(self, x):
        B, N, c, ich = x.shape  # (B, N, c, k)
        assert ich == 2 * self.k
        x_e = torch.matmul(x, self.rc_e.double())
        x_o = torch.matmul(x, self.rc_o.double())
